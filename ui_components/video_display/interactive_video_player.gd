...
func update_playback_settings() -> void:
	set_muted(GameSettings.profile_settings.video_muted)
	set_volume(GameSettings.profile_settings.video_volume)
	video_playback.set_playback_speed(GameSettings.profile_settings.video_speed)
	still_setup = true
	speed_button.select(int(GameSettings.profile_settings.video_speed / 0.25 - 1))
	still_setup = false

func _on_speed_button_selected(idx:int) -> void:
	if still_setup:
		return
	GameSettings.profile_settings.video_speed = 0.25 * (idx + 1)
...
func toggle_mute() -> void:
	GameSettings.profile_settings.video_muted = !GameSettings.profile_settings.video_muted
	GameSettings.save_profile_settings()
...
func _on_volume_slider_stop(_val) -> void:
	...
	GameSettings.profile_settings.video_volume = volume_slider.value
	GameSettings.save_profile_settings()
...
func force_pause() -> void:
	...
	play_button.tooltip_text = play_tooltip
	if GameSettings.profile_settings.no_hum_on_video:
		ProgramStatus.set_hum_playing(true)
...
func force_play() -> void:
	...
	if GameSettings.profile_settings.no_hum_on_video:
		ProgramStatus.set_hum_playing(false)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_released("pause_video"):
		...
		GameSettings.profile_settings.video_volume = min(1.0, volume_slider.value + 0.1)
		GameSettings.save_profile_settings()
	elif event.is_action_released("volume_down"):
		GameSettings.profile_settings.video_volume = max(0.0, volume_slider.value - 0.1)
		GameSettings.save_profile_settings()
