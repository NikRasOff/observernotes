...
## Deletes a directory completely
static func demolish_directory(path:String) -> void:
	var dirs:Array[String] = [path]
	while !dirs.is_empty():
		for f in DirAccess.get_files_at(dirs.back()):
			DirAccess.remove_absolute(f)
		var new_dirs := DirAccess.get_directories_at(dirs.back())
		if new_dirs.is_empty():
			DirAccess.remove_absolute(dirs.pop_back())
		else:
			for d in new_dirs:
				dirs.append(d)
